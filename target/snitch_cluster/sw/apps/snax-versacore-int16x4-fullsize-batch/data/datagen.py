...
# Data generator for dual VersaCore int16x4 full-size batch test
# Sequential N-tiling (no double-buffering) to fit B tiles in TCDM.
# Mode 0: A[8,2048] @ W[2048,1408], V[2048,1408] → SwiGLU → Out0[8,1408]
# Mode 1: A'=Out0[8,1408] @ W2l[1408,1024], W2r[1408,1024]
# B0/B1 memory reused for W2l/W2r in Mode 1.
...
def emit_dual_versacore_data(**kwargs):
    ...
    for i in range(3, 6):
        data_str += [format_scalar_definition("int32_t", f"Atlbound{i}", 1)]
        data_str += [format_scalar_definition("int32_t", f"Atlstride{i}", 0)]

    A_enabled_channel_CSR_num = int(math.ceil(a_array_width // bankWidth / 32))
    channel_en_A = gen_channel_enable_CSR([0] * A_enabled_channel_CSR_num, int(meshRow * tileSize * a_len // bankWidth))
    ...
    data_str += [format_scalar_definition("int32_t", "B0tlbound2", M)]
    data_str += [format_scalar_definition("int32_t", "B0tlstride2", 0)]
    data_str += [format_scalar_definition("int32_t", "B0tlbound3", 1)]
    data_str += [format_scalar_definition("int32_t", "B0tlstride3", 0)]

    channel_en_B0 = gen_channel_enable_CSR([0] * B_enabled_channel_CSR_num, channel_en_B0_bits)
    data_str += ["int32_t channel_en_B0[] = { " + ", ".join(map(str, channel_en_B0)) + " };"]
    ...
    data_str += [format_scalar_definition("int32_t", "B1tlbound2", M)]
    data_str += [format_scalar_definition("int32_t", "B1tlstride2", 0)]
    data_str += [format_scalar_definition("int32_t", "B1tlbound3", 1)]
    data_str += [format_scalar_definition("int32_t", "B1tlstride3", 0)]

    channel_en_B1 = gen_channel_enable_CSR([0] * B_enabled_channel_CSR_num, channel_en_B0_bits)
    data_str += ["int32_t channel_en_B1[] = { " + ", ".join(map(str, channel_en_B1)) + " };"]
    ...
    data_str += [format_scalar_definition("int32_t", "Dtlbound0", 1)]
    data_str += [format_scalar_definition("int32_t", "Dtlstride0", d_spatial_bound_0 * (bankWidth // 8))]
    data_str += [format_scalar_definition("int32_t", "Dtlbound1", N_chunk)]
    data_str += [format_scalar_definition("int32_t", "Dtlstride1", Dtlstride1)]
    data_str += [format_scalar_definition("int32_t", "Dtlbound2", M)]
    data_str += [format_scalar_definition("int32_t", "Dtlstride2", N_chunk * Dtlstride1)]
    ...
    data_str += ["int32_t channel_en_D0[] = { " + ", ".join(map(str, channel_en_D0)) + " };"]
    data_str += ["int32_t channel_en_D1[] = { " + ", ".join(map(str, channel_en_D1)) + " };"]

    mode0_output_elems = M * N * meshRow * meshCol
    data_str += [format_scalar_definition("int32_t", "mode0_output_elems", mode0_output_elems)]

    # ===================== Base addresses (single B buffers) ==================
    ...
    b_chunk_alloc = max(b_chunk_data_length, b_chunk_access_range)
    ...
    # Single B0 buffer
    delta_local_b0 = a_data_length_bytes
    delta_local_b0 = align_wide_addr(delta_local_b0, granularity_b * bankWidth // 8)

    # Single B1 buffer
    delta_local_b1 = delta_local_b0 + b_chunk_alloc
    ...
    m1_d_full_max_temporal = max(0, (N1 - 1) * M1_Dtlstride1)
    ...
    delta_local_mode1_d0 = delta_local_d1_mode0 + d_full_alloc
    ...
    # Verify TCDM fits (8MB)
    total_used = delta_local_mode1_d1 + m1_d_full_alloc
    assert total_used <= 8388608, f"TCDM overflow: {total_used} > 8388608"
    ...
    data_str += [format_scalar_definition("int32_t", "M1_Atlbound1", N1_chunk)]
    data_str += [format_scalar_definition("int32_t", "M1_Atlstride1", 0)]
    data_str += [format_scalar_definition("int32_t", "M1_Atlbound2", M1)]
    data_str += [format_scalar_definition("int32_t", "M1_Atlstride2", int(K1 * a_len * tileSize * meshRow // 8))]

    M1_B0tlstride0 = b_tile_padded
    M1_B0tlstride1 = K1 * b_tile_padded

    data_str += [format_scalar_definition("int32_t", "M1_B0tlbound0", K1)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlstride0", M1_B0tlstride0)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlbound1", N1_chunk)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlstride1", M1_B0tlstride1)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlbound2", M1)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlstride2", 0)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlbound3", 1)]
    data_str += [format_scalar_definition("int32_t", "M1_B0tlstride3", 0)]

    data_str += [format_scalar_definition("int32_t", "M1_B1tlbound0", K1)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlstride0", M1_B0tlstride0)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlbound1", N1_chunk)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlstride1", M1_B0tlstride1)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlbound2", M1)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlstride2", 0)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlbound3", 1)]
    data_str += [format_scalar_definition("int32_t", "M1_B1tlstride3", 0)]

    data_str += [format_scalar_definition("int32_t", "M1_Dtlbound0", 1)]
    data_str += [format_scalar_definition("int32_t", "M1_Dtlstride0", d_spatial_bound_0 * (bankWidth // 8))]
    data_str += [format_scalar_definition("int32_t", "M1_Dtlbound1", N1_chunk)]
    data_str += [format_scalar_definition("int32_t", "M1_Dtlstride1", M1_Dtlstride1)]
    data_str += [format_scalar_definition("int32_t", "M1_Dtlbound2", M1)]
    data_str += [format_scalar_definition("int32_t", "M1_Dtlstride2", N1_chunk * M1_Dtlstride1)]
    ...
    data_str += [format_scalar_definition("int32_t", "m1_b_chunk_data_length", m1_b_chunk_data_length)]
    ...
    W_packed = pad_b_tiles(pack_int4(W_int4), N * K, b_tile_raw, b_tile_padded)
    V_packed = pad_b_tiles(pack_int4(V_int4), N * K, b_tile_raw, b_tile_padded)
    ...
    # ===================== Mode 0 Golden =====================================
    vc0_int32 = block_gemm_int16x4(M, K, N, meshRow, tileSize, meshCol,
                                    A_int16, W_int4, subtraction_a, subtraction_b)
    vc1_int32 = block_gemm_int16x4(M, K, N, meshRow, tileSize, meshCol,
    ...
    W2_left_packed = pad_b_tiles(pack_int4(W2_left_int4), N1 * K1, b_tile_raw, b_tile_padded)
    W2_right_packed = pad_b_tiles(pack_int4(W2_right_int4), N1 * K1, b_tile_raw, b_tile_padded)
    ...
    golden_d0_int32 = block_gemm_int16x4(M1, K1, N1, meshRow, tileSize, meshCol,
                                          mode1_A_flat, W2_left_int4,
                                          subtraction_a, subtraction_b)
    golden_d1_int32 = block_gemm_int16x4(M1, K1, N1, meshRow, tileSize, meshCol,
                                          mode1_A_flat, W2_right_int4,
                                          subtraction_a, subtraction_b)
