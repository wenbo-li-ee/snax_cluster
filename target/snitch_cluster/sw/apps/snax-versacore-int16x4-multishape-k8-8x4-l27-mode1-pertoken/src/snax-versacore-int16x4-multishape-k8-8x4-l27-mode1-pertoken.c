// L27 layout (A pad 32 B + B1 bank 34) + Mode1 per-token output layout.
// Derived from s9 (mode1-pertoken). Mode0 uses L27 A/B placement.
...
static int run_shape(const shape_cfg_t *cfg) {
    ...
    // Mode 0: L27 A (padded rows, bank-rotating) + L27 B1 (bank-34-colored).
    ...
    // Mode 1: per-token D writer layout (tbound/tstride per s9 spec).
...
int main() {
    ...
    if (shape_cfg[0].tcdm_end > TCDM_CAPACITY_BYTES) {
        printf("K8 L27 mode1-pertoken TCDM placement exceeds capacity: end=%d cap=%d\n",
               shape_cfg[0].tcdm_end, TCDM_CAPACITY_BYTES);
        return 1;
    }

    if (snrt_global_core_idx() == 0) {
        printf("L27 mode1-pertoken multishape K8 8x4 staging: bytes=%d\n",
    ...
    if (snrt_global_core_idx() == 0) {
        printf("L27 mode1-pertoken multishape K8 8x4 shared dataset ready\n");
    }
    ...
    if (snrt_global_core_idx() == 0) {
        for (int s = 0; s < NUM_SHAPES; s++) {
            int r = run_shape(&shape_cfg[s]);
            if (r) {
                printf("Shape %d returned error %d — stopping.\n", s, r);
                err += r;
                break;
            }
        }
        printf("L27 mode1-pertoken K8 8x4 total checks: 9, total error: %d\n", err);
    }
