#!/usr/bin/env python3
# L27 layout (A pad 32 B + B1 bank 34) combined with Mode1 per-token output layout.
# Base: s9 (mode1-pertoken) datagen, with placement formula from layout-explore L27.
...
# L27 layout parameters
A_PAD = 32          # bytes of per-row padding appended after K0 data
B1_COLOR = 272      # bank-coloring offset for B1 (= bank 34 * 8 bytes)
...
def make_physical_a(m_total, k_total):
    """Generate activation data — same pattern as s8/s9 baseline."""
...
def make_padded_a(logical_a, a_row_stride_bytes):
    """Embed logical_a rows into wider rows (A_PAD zero-padding at tail)."""
    row_elems = a_row_stride_bytes // 2
...
def colored_offset(offset, color_bytes=0, alignment=1024):
    """Align offset up to 1 KB boundary, then add color_bytes bank offset."""
...
def build_shape_cfg(shape, globals_, golden_names):
    ...
    m_total = globals_["m_total"]

    # L27: A row stride = K0_bytes + A_PAD
    a_row_stride = k0_bytes + A_PAD  # 4096 + 32 = 4128
    ...
    # Mode1 D writer: per-token layout (s9 spec)
    beats_per_row = mesh_col // 4
    mode1_token_stride = n1_total * 2  # = 2048 bytes per token

    # Tensor placement — L27 layout (colored_offset with B1_COLOR=272)
    a_bytes = m_total * a_row_stride
    w_bytes = k0_s0_tiles * n0_s0_tiles * 16
    mode0_d_bytes = m_total * n0_total * 2
    w2_bytes = k1_s0_tiles * n1_s0_tiles * 16
    mode1_d_bytes = m_total * n1_total * 2

    delta_local_a = 0
    delta_local_b0 = colored_offset(delta_local_a + a_bytes, 0)
    delta_local_b1 = colored_offset(delta_local_b0 + w_bytes, B1_COLOR)
    delta_local_d0 = colored_offset(delta_local_b1 + w_bytes, 0)
    delta_local_w2l = colored_offset(delta_local_d0 + mode0_d_bytes, 0)
    delta_local_w2r = colored_offset(delta_local_w2l + w2_bytes, 0)
    delta_local_mode1_d0 = colored_offset(delta_local_w2r + w2_bytes, 0)
    ...
    fields = [
        ...
        f".K1 = {k1_tiles}",
        f".N1 = {n1_tiles}",
        # A sstride[1] reflects padded row stride
        ...
        # A tstride[2] (M dimension) uses padded row stride
...
def emit_header(params):
    ...
    # L27: A stored with per-row padding
    a_row_stride = k0_bytes + A_PAD
    padded_a = make_padded_a(physical_a, a_row_stride)
    ...
    arrays = []
    # A array includes per-row padding (zero-fill after K0 data)
    arrays.append(emit_i16_array("A", padded_a))
    ...
    for shape in SHAPE_DIMS:
        ...
        # Golden uses padded A; spatial stride must use a_row_stride (4128), not k0_bytes (4096)
        a_flat = streamer_i16_flat(
            padded_a, m_shape_tiles, k_shape_tiles, A_SPATIAL_BOUNDS,
            [8, a_row_stride], tile_size * 2, mesh_row * a_row_stride, a_channel_en)
        ...
        # Mode1 D writer per-token: transpose (M, N_tiles, meshRow, meshCol)
        # → (M, meshRow, N_tiles, meshCol) so each token's output is contiguous.
    ...
    return "\n".join([
        "#include <stdint.h>",
        "",
        "#ifndef SNAX_VERSACORE_INT16X4_MULTISHAPE_K8_8X4_L27_MODE1_PERTOKEN_DATA_H",
        "#define SNAX_VERSACORE_INT16X4_MULTISHAPE_K8_8X4_L27_MODE1_PERTOKEN_DATA_H",
        "",
        "/* Layout: L27 — A pad 32 B (a_row_stride=4128), B1 bank 34 (color=272 B) */",
        "/* Mode1 D writer: per-token contiguous output (spec s9) */",
        "",
        "#define NUM_SHAPES 3",
        "#define DATA_TYPE 0",
        ...
        f"#define A_DATA_LENGTH {m_total * a_row_stride}",  # bytes, matches s9 convention
