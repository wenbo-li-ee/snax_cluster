// Multi-shape 16x2 Mode0 single-writer test for dual VersaCore int16x4 SwiGLU.
...
int main() {
    ...
    if (snrt_global_core_idx() == 0) {
        printf("Mode0 single-writer multishape 16x2 staging shared dataset: bytes=%d\n",
    ...
    if (snrt_global_core_idx() == 0) {
        printf("Mode0 single-writer multishape 16x2 shared dataset ready\n");
        for (int shape = 0; shape < NUM_SHAPES; shape++) {
            err += run_shape(&shape_cfg[shape]);
        }
        printf("Mode0 single-writer multishape 16x2 total checks: 9, total error: %d\n", err);
    }
