// Int16x4 Shape S2 ping-pong (B tile double-buffering) test
// array_shape=2 → (meshRow=2, tileSize=8, meshCol=16)
// Mode 0 (SwiGLU) + Mode 1 (GEMM) with independent A1
// Double-buffering: split N dimension into chunks, alternate B buffers.
...
int main() {
    ...
    // Mode 1 buffers
    int16_t *local_a1 = (int16_t *)(snrt_l1_next() + delta_local_a1);
    ...
    // ================================================================
    // DMA: preload A, A1, + first B chunk for Mode 0
    // ================================================================
    if (snrt_is_dm_core()) {
        snrt_dma_start_1d(local_a, A, a_data_length);
        snrt_dma_start_1d(local_a1, A1, a1_data_length);
