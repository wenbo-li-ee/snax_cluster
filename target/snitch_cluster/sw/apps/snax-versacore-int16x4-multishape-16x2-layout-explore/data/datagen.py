...
LAYOUTS = [
    ...
    {"id": 4, "name": "a_pad_88b_bank_inc11", "a_pad": 88,
     "color": False, "b_pad": 0},
    {"id": 5, "name": "a_pad_104b_bank_inc13", "a_pad": 104,
     "color": False, "b_pad": 0},
    {"id": 6, "name": "a_pad_120b_bank_inc15", "a_pad": 120,
     "color": False, "b_pad": 0},
    {"id": 7, "name": "a_pad_136b_bank_inc17", "a_pad": 136,
     "color": False, "b_pad": 0},
    {"id": 8, "name": "a_pad_152b_bank_inc19", "a_pad": 152,
     "color": False, "b_pad": 0},
    {"id": 9, "name": "a_pad_136b_colored_bases", "a_pad": 136,
     "color": True, "b_pad": 0},
    {"id": 10, "name": "a_pad_136b_colored_bases_bnpad64", "a_pad": 136,
     "color": True, "b_pad": 64},
]
...
def packed_int4_tiled(n_tiles, k_s0_tiles, n_stride, value):
    assert 0 <= value <= 7
    packed_byte = (value << 4) | value
    out = np.zeros(n_tiles * n_stride, dtype=np.uint8)
    live_bytes = k_s0_tiles * 16
    for n in range(n_tiles):
        out[n * n_stride:n * n_stride + live_bytes] = packed_byte
    return out
...
def align_to_bank(offset, bank, alignment=16, bank_word_bytes=8, banks=64):
    out = align_up(offset, alignment)
    while (out // bank_word_bytes) % banks != bank:
        out += alignment
    return out
...
def a_sstride_for_shape(array_shape, row_stride):
    return {
        0: [8, row_stride, 2 * row_stride, 4 * row_stride],
        1: [8, 32, row_stride, 2 * row_stride],
        2: [8, 32, 64, row_stride],
    }[array_shape]
...
def place_tensors(globals_, layout):
    ...
    w_bytes = globals_["n0_tiles"] * globals_["b0_n_stride"]
    mode0_d_bytes = globals_["m_total"] * globals_["n0_total"] * 2
    w2_bytes = globals_["n1_tiles"] * globals_["w2_n_stride"]
    mode1_d_bytes = globals_["m_total"] * globals_["n1_total"] * 2

    delta_local_a = 0
    if layout["color"]:
        delta_local_b0 = align_to_bank(delta_local_a + a_bytes, 32)
        delta_local_b1 = align_to_bank(delta_local_b0 + w_bytes, 40)
        delta_local_d0 = align_to_bank(delta_local_b1 + w_bytes, 48)
        delta_local_w2l = align_to_bank(delta_local_d0 + mode0_d_bytes, 32)
        delta_local_w2r = align_to_bank(delta_local_w2l + w2_bytes, 40)
        delta_local_mode1_d0 = align_to_bank(delta_local_w2r + w2_bytes, 48)
        delta_local_mode1_d1 = align_to_bank(delta_local_mode1_d0 + mode1_d_bytes, 56)
    else:
...
def build_shape_cfg(shape, globals_, golden_names, layout, placement):
    ...
    n0_tiles = globals_["n0_tiles"]
    n1_tiles = globals_["n1_tiles"]
    a_row_stride = globals_["k0_bytes"] + layout["a_pad"]

    m_tiles = 1
    k_tiles = k0_total // tile_size
    k1_tiles = k1_total // tile_size
    b_k_stride = (tile_size // 16) * 16
    ...
    mode0_a_sstride = a_sstride_for_shape(array_shape, a_row_stride)
    ...
    fields = [
        ...
        f".mode1_A_sstride = {c_array([8, 32, 64, 128])}",
        f".B_sstride = {c_array([8, 16, 32])}",
        ...
        f".mode1_A_tstride = {c_array([256, 0, mode0_d_m_stride, 0, 0, 0])}",
        f".mode0_B_tbound = {c_array([k_tiles, n0_tiles, m_tiles, 1])}",
        f".mode0_B_tstride = {c_array([b_k_stride, globals_['b0_n_stride'], 0, 0])}",
        f".mode1_B_tbound = {c_array([k1_tiles, n1_tiles, m_tiles, 1])}",
        f".mode1_B_tstride = {c_array([b_k_stride, globals_['w2_n_stride'], 0, 0])}",
        f".mode0_D_tbound = {c_array([d_bound0, n0_tiles, m_tiles, 1])}",
        f".mode0_D_tstride = {c_array([8, d_stride1, mode0_d_m_stride, 0])}",
        f".mode1_D_tbound = {c_array([d_bound0, n1_tiles, m_tiles, 1])}",
        f".mode1_D_tstride = {c_array([8, d_stride1, mode1_d_m_stride, 0])}",
        f".A_channel_en = {c_array([-1])}",
...
def build_golden_arrays(logical_a, params):
    k0_total = params["k0_total"]
    n0_total = params["n0_total"]
    k1_total = params["k1_total"]
    n1_total = params["n1_total"]
    k0_bytes = params["k0_bytes"]
    n0_tiles = params["n0_tiles"]
    arrays = []
    names_by_shape = {}
    for shape in SHAPE_DIMS:
        ...
        mode0_a_sstride = a_sstride_for_shape(array_shape, k0_bytes)
        a_flat = streamer_i16_flat(
            logical_a, m_shape_tiles, k_shape_tiles, A_SPATIAL_BOUNDS,
            mode0_a_sstride, tile_size * 2, mesh_row * k0_bytes)
        ...
        mode1_a_flat = streamer_i16_flat(
            mode0, m_shape_tiles, k1_tiles, A_SPATIAL_BOUNDS, [8, 32, 64, 128],
            256, mode0_d_m_stride)
        ...
        arrays.append(emit_i16_array(names[1], mode1_d0))
        arrays.append(emit_i16_array(names[2], mode1_d1))
    return arrays, names_by_shape


def bank_coverage_for_shape(array_shape, row_stride):
    offsets = spatial_offsets(A_SPATIAL_BOUNDS, a_sstride_for_shape(array_shape, row_stride))
...
def emit_header(params):
    ...
    k0_s0_tiles = k0_total // 16
    k1_s0_tiles = k1_total // 16
    n0_tiles = n0_total // 2
    n1_tiles = n1_total // 2
    k0_bytes = k0_total * 2
    ...
    weight_arrays = {}
    for b_pad in sorted({layout["b_pad"] for layout in LAYOUTS}):
        b0_n_stride = k0_s0_tiles * 16 + b_pad
        w2_n_stride = k1_s0_tiles * 16 + b_pad
        suffix = f"bpad_{b_pad}"
        weight_arrays[b_pad] = {
            "W": f"W_{suffix}",
            "V": f"V_{suffix}",
            "W2_left": f"W2_left_{suffix}",
            "W2_right": f"W2_right_{suffix}",
            "b_len": n0_tiles * b0_n_stride,
            "w2_len": n1_tiles * w2_n_stride,
            "b0_n_stride": b0_n_stride,
            "w2_n_stride": w2_n_stride,
        }
        arrays.append(emit_u8_array(f"W_{suffix}",
                                    packed_int4_tiled(n0_tiles, k0_s0_tiles,
                                                      b0_n_stride, 1)))
        arrays.append(emit_u8_array(f"V_{suffix}",
                                    packed_int4_tiled(n0_tiles, k0_s0_tiles,
                                                      b0_n_stride, 2)))
        arrays.append(emit_u8_array(f"W2_left_{suffix}",
                                    packed_int4_tiled(n1_tiles, k1_s0_tiles,
                                                      w2_n_stride, 1)))
        arrays.append(emit_u8_array(f"W2_right_{suffix}",
                                    packed_int4_tiled(n1_tiles, k1_s0_tiles,
                                                      w2_n_stride, 2)))

    golden_arrays, golden_names = build_golden_arrays(logical_a, base_params)
    arrays.extend(golden_arrays)

    layout_inits = []
    layout_notes = []
    for layout in LAYOUTS:
        weights = weight_arrays[layout["b_pad"]]
        globals_ = {
            **base_params,
            "b0_n_stride": weights["b0_n_stride"],
            "w2_n_stride": weights["w2_n_stride"],
        }
        ...
        layout_inits.append("\n".join([
            ...
            f"        .w_data = {weights['W']},",
            f"        .v_data = {weights['V']},",
            f"        .w2_left_data = {weights['W2_left']},",
            f"        .w2_right_data = {weights['W2_right']},",
            f"        .b_data_length = {weights['b_len']},",
            f"        .w2_data_length = {weights['w2_len']},",
            "        .shapes = {",
            shape_cfgs,
            "        }",
            "    }",
        ...
        layout_notes.append(
            ...
            f"{k0_bytes + layout['a_pad']} B, B N stride "
            f"{weights['b0_n_stride']} B, W2 N stride "
            f"{weights['w2_n_stride']} B, colored={layout['color']}; "
            + "; ".join(coverage))

    return "\n".join([
        "#include <stdint.h>",
        "",
        "#ifndef SNAX_VERSACORE_INT16X4_MULTISHAPE_16X2_LAYOUT_EXPLORE_DATA_H",
        "#define SNAX_VERSACORE_INT16X4_MULTISHAPE_16X2_LAYOUT_EXPLORE_DATA_H",
        "",
        ...
        "    int32_t mode0_A_sstride[4], mode1_A_sstride[4], B_sstride[3], D_sstride[1];",
