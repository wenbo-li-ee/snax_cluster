// Multi-layout 16x2 data-placement exploration for dual VersaCore int16x4 SwiGLU.
...
static int wait_accel_diag(int layout, int shape, int mode) {
    ...
    while (csrr_ss(DUAL_VC_BUSY)) {
        if ((uint32_t)(snrt_mcycle() - start) > WAIT_TIMEOUT_CYCLES) {
            printf("L%d S%d Mode%d accelerator wait timeout: busy=%u\n",
...
static int run_shape(int layout, const shape_cfg_t *cfg) {
    ...
    set_dual_versacore_streamer_csr_d0_only(
        ...
        cfg->delta_local_b0, cfg->B_sstride, cfg->mode0_B_tbound,
        cfg->mode0_B_tstride, SET_ADDR_REMAP_INDEX_B0, cfg->B_channel_en,
        cfg->delta_local_b1, cfg->B_sstride, cfg->mode0_B_tbound,
    ...
    set_dual_versacore_streamer_csr(
        ...
        cfg->delta_local_w2l, cfg->B_sstride, cfg->mode1_B_tbound,
        cfg->mode1_B_tstride, SET_ADDR_REMAP_INDEX_B0, cfg->B_channel_en,
        cfg->delta_local_w2r, cfg->B_sstride, cfg->mode1_B_tbound,
    ...
    printf("L%d S%d Mode1 D0: %s, Error: %d\n", layout, cfg->array_shape,
           err_m1_d0 ? "FAIL" : "PASS", err_m1_d0);
    printf("L%d S%d Mode1 D1: %s, Error: %d\n", layout, cfg->array_shape,
...
static int stage_layout(const layout_cfg_t *layout) {
    ...
    if (snrt_global_core_idx() == 0) {
        printf("L%d %s staging dataset: bytes=%d A_stride=%d B0_bank=%d B1_bank=%d\n",
...
int main() {
    ...
    if (snrt_global_core_idx() == 0) {
        printf("Layout exploration: layouts=%d shapes=%d\n", NUM_LAYOUTS,
               NUM_SHAPES);
    }

    for (int layout = 0; layout < NUM_LAYOUTS; layout++) {
        const layout_cfg_t *layout_cfg = &layout_cfgs[layout];
        ...
        if (snrt_global_core_idx() == 0) {
            ...
            for (int shape = 0; shape < NUM_SHAPES; shape++) {
                err += run_shape(layout_cfg->layout_id,
                                 &layout_cfg->shapes[shape]);
            }
            printf("L%d %s total checks: 9, total error so far: %d\n",
                   layout_cfg->layout_id, layout_cfg->name, err);
        }
        snrt_cluster_hw_barrier();
    }
