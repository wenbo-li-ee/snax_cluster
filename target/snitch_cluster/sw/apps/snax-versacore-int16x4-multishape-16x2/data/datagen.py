...
def packed_int4_ones(num_shape0_tiles):
    # Tight S0 layout: one [tileSize=16, meshCol=2] int4 tile is 32 nibbles = 16 B.
    return np.full(num_shape0_tiles * 16, 0x11, dtype=np.uint8)
...
def build_shape_cfg(shape, globals_, golden_names):
    ...
    w_bytes = k0_s0_tiles * n0_tiles * 16
    mode0_d_bytes = 8 * n0_total * 2
    w2_bytes = k1_s0_tiles * n1_tiles * 16
    ...
    mode0_a_sstride = {
        0: [8, k0_bytes, 2 * k0_bytes, 4 * k0_bytes],
        1: [8, 32, k0_bytes, 2 * k0_bytes],
        2: [8, 32, 64, k0_bytes],
    }[array_shape]
    ...
    fields = [
        ...
        f".mode0_B_tstride = {c_array([b_k_stride, k0_s0_tiles * 16, 0, 0])}",
        f".mode1_B_tbound = {c_array([k1_tiles, n1_tiles, m_tiles, 1])}",
        f".mode1_B_tstride = {c_array([b_k_stride, k1_s0_tiles * 16, 0, 0])}",
...
def emit_header(params):
    ...
    arrays.append(emit_u8_array("W", packed_int4_ones(k0_s0_tiles * n0_tiles)))
    arrays.append(emit_u8_array("V", packed_int4_ones(k0_s0_tiles * n0_tiles)))
    arrays.append(emit_u8_array("W2_left", packed_int4_ones(k1_s0_tiles * n1_tiles)))
    arrays.append(emit_u8_array("W2_right", packed_int4_ones(k1_s0_tiles * n1_tiles)))
    ...
    for shape in SHAPE_DIMS:
        ...
        w2_flat = np.ones(n1_tiles_shape * k1_tiles * mesh_col * tile_size, dtype=np.int8)
        d_bound0 = mesh_row * mesh_col // 4
        d_stride1 = d_bound0 * 8
        mode0_d_m_stride = n0_tiles * d_stride1
    ...
    return "\n".join([
        "#include <stdint.h>",
        "",
        "#ifndef SNAX_VERSACORE_INT16X4_MULTISHAPE_16X2_DATA_H",
        "#define SNAX_VERSACORE_INT16X4_MULTISHAPE_16X2_DATA_H",
        "",
        "#define NUM_SHAPES 3",
        "#define DATA_TYPE 0",
        ...
        f"#define B_DATA_LENGTH {k0_s0_tiles * n0_tiles * 16}",
        f"#define W2_DATA_LENGTH {k1_s0_tiles * n1_tiles * 16}",
