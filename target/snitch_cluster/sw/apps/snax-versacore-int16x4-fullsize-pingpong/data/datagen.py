...
# Data generator for dual VersaCore int16x4 scaled 1/16 ping-pong test
# Same workload as scale16-batch but with B tile double-buffering along N dimension.
...
def emit_dual_versacore_data(**kwargs):
    ...
    # W2l/W2r OVERLAY Mode 0 B buffers (Mode 0 B no longer needed during Mode 1)
    # W2l uses B0 buffer slots, W2r uses B1 buffer slots
    delta_local_w2l_buf0 = delta_local_b0_buf0
    delta_local_w2l_buf1 = delta_local_b0_buf1
    delta_local_w2r_buf0 = delta_local_b1_buf0
    delta_local_w2r_buf1 = delta_local_b1_buf1

    # Mode 1 output (placed after D1_mode0)
    ...
    m1_d_full_max_temporal = max(0, (M1_Dtlbound0 - 1) * M1_Dtlstride0) \
        + max(0, (N1 - 1) * M1_Dtlstride1) + max(0, (M1 - 1) * (N1 * out_elem_bits * meshRow * meshCol // 8))
    ...
    # ===================== Mode 1 (GEMM) ===================================
    # Mode 1 A = Mode 0 output, contiguous in memory → simple reshape
