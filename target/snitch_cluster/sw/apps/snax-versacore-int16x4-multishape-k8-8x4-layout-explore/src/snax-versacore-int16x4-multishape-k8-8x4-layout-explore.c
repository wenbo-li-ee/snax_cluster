// Multi-layout K=8 8x4 data-placement exploration for dual VersaCore int16x4 SwiGLU.
...
int main() {
    ...
    if (snrt_global_core_idx() == 0) {
        printf("K8 8x4 layout exploration: layouts=%d shapes=%d\n",
...
#if RUN_MODE1
    printf("L%d %s total checks: 9, total error: %d\n", layout_cfg->layout_id,
           layout_cfg->name, err);
#else
    printf("L%d %s total checks: 3, total error: %d\n", layout_cfg->layout_id,
           layout_cfg->name, err);
#endif
