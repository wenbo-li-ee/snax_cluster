// Multi-shape K=8 8x4 Mode0 single-writer test for dual VersaCore int16x4 SwiGLU.
...
int main() {
    ...
    if (shape_cfg[0].tcdm_end > TCDM_CAPACITY_BYTES) {
        printf("K8 8x4 TCDM placement exceeds capacity: end=%d cap=%d\n",
               shape_cfg[0].tcdm_end, TCDM_CAPACITY_BYTES);
        return 1;
    }

    if (snrt_global_core_idx() == 0) {
        printf("Mode0 single-writer multishape K8 8x4 staging shared dataset: bytes=%d\n",
    ...
    if (snrt_global_core_idx() == 0) {
        printf("Mode0 single-writer multishape K8 8x4 shared dataset ready\n");
        for (int shape = 0; shape < NUM_SHAPES; shape++) {
            err += run_shape(&shape_cfg[shape]);
        }
        printf("Mode0 single-writer multishape K8 8x4 total checks: 9, total error: %d\n", err);
    }
