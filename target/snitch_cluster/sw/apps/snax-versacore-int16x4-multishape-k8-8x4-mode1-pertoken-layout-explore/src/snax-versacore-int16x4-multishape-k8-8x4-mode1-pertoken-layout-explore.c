// Multi-layout K=8 8x4 Mode1 per-token data-placement exploration for dual VersaCore int16x4 SwiGLU.
...
#if RUN_MODE1
    // Mode 1: read Mode0 D0 and write per-token contiguous D0/D1 outputs.
...
static int stage_layout(const layout_cfg_t *layout) {
    ...
    if (snrt_global_core_idx() == 0) {
         printf("L%d %s staging dataset: bytes=%d A_stride=%d B0_bank=%d B1_bank=%d D0_bank=%d\n",
...
int main() {
    ...
    if (snrt_global_core_idx() == 0) {
        printf("K8 8x4 Mode1 per-token layout exploration: layouts=%d shapes=%d\n",
    ...
    if (snrt_global_core_idx() != 0) {
        ...
        int checks = (shape_end - shape_begin) * (RUN_MODE1 ? 3 : 1);
